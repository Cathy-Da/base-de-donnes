# geolocate_from_correction.R — force le géocodage ArcGIS pour TOUTES les lignes
# Source = nasa_disaster_correction.csv (schéma: id,country,geolocation,level,adm1,location,disastertype,continent)
...
CHUNK_SIZE   <- 200   # ajuste à 100–300 si besoin
MAX_RETRIES  <- 3
SLEEP_RETRY  <- 2     # backoff simple: r * SLEEP_RETRY
...
geo_arcgis_retry <- function(addr_vec, max_retries = MAX_RETRIES, sleep_base = SLEEP_RETRY){
  for (r in seq_len(max_retries)) {
    res <- tryCatch(
      ...
      error = function(e){ message(sprintf("   ⚠️ ArcGIS erreur (retry %d/%d): %s", r, max_retries, conditionMessage(e))); NULL }
    )
